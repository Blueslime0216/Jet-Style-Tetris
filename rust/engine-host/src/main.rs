...
use tetr_core::ai::{Placement, SearchState, generate, generate_with_hold, placement_to_inputs};
    ...
    for p in placements(state, used) {
        ...
        if let Some(index) = targets.iter().position(|(kind, wanted)| {
            *kind == p.piece_type() && wanted.len() == 4 && cells.iter().all(|c| wanted.contains(c))
            ...
            if let Some(mut route) = template_route(&next, false, &remaining, budget) {
    ...
    for _ in 0..depth.min(3) {
        ...
        for (node, route, used) in frontier {
            for p in placements(&node, used) {
                ...
                {
                    found.push(json!({"id":format!("{}-{}",g,found.len()),"name":match g {"pc"=>"Perfect clear route","tsd"=>"T-spin double route",_=>"T-spin triple route"},"strategy":g,"route":path}));
                }
...
impl Host {
    ...
    fn view(&self) -> Value {
        json!({"frame":self.frame,"locks":self.locks,"seats":self.games.iter().map(|g|snapshot(&g.snapshot())).collect::<Vec<_>>()})
    }
    fn command(&mut self, v: &Value) -> Result<Value, String> {
        ...
        match v["op"].as_str().unwrap_or("") {
            ...
            "tick" => {
                ...
                for s in 0..2 {
                    let ev = self.games[s].step(input(&v["inputs"][s]));
            ...
            "plans" => {
                ...
                if let Some(templates) = v["templates"].as_array() {
                    for template in templates.iter().take(24) {
                        ...
                        let all: Vec<_> = pieces
                            ...
                            .collect();
                        if state.board.cell_coords().iter().any(|c| !all.contains(c)) {
                            valid = false;
                        }
                        if !valid || targets.is_empty() {
                            continue;
                        }
                        let mut budget = 1500;
                        if let Some(route) = template_route(
                            ...
                            plans.push(json!({"id":template["id"],"name":template["name"],"strategy":template["strategy"],"source":template["source"],"route":route}));
                        }
                    }
                }
                Ok(json!(plans))
            ...
            "candidates" => {
                ...
                Ok(json!(
                    placements(&state, snap.active.as_ref().is_some_and(|a| a.hold_used))
                        .iter()
            ...
            "place" => {
                ...
                let p = placements(&state, snap.active.as_ref().is_some_and(|a| a.hold_used))
                    .into_iter()
                    .find(|p| Some(id(p, &state).as_str()) == v["candidate"].as_str())
                    .ok_or("placement no longer reachable")?;
                let expected = candidate(&p, &state);
                let frames = placement_to_inputs(&state.board.to_array2d(), &state.active, &p);
                let mut events = Vec::new();
                for f in frames {
...
mod tests {
    ...
    fn candidates_execute_exactly() {
        for seed in [1, 17, 82719451] {
            ...
            for p in placements(&state, false) {
